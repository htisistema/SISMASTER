...
WHILE .T.
        ...
        IF LASTKEY() = 27
                ...
                RETURN .T.
        ENDIF
        ccomm :="SELECT * FROM mastprod WHERE codigo IS NOT NULL "
        IF ! EMPTY(mgrupo)
                cons_merc := {}
        ...
        ENDIF
        IF msaldo = 'C'
                ccomm := ccomm + " AND (saldo IS NOT NULL OR saldo > "+sr_cdbvalue(0)+")"
        ELSEIF msaldo = 'S'
                ccomm := ccomm + " AND (saldo IS NULL OR saldo <= "+sr_cdbvalue(0)+")"
        ENDIF
        ...
        IF mordem # 'G'
                ...
                FOR i = 1 TO LEN(cons_merc)
                        DEVPOS(PROW()+1,00);DEVOUT(STR(cons_merc[i,1],5))
        ...
        conf_impressao(ALLTRIM(m_cfg[12])+'HTI.REL')
ENDDO
wvw_lclosewindow()
RETURN NIL
*********************** f i m ***********************************
